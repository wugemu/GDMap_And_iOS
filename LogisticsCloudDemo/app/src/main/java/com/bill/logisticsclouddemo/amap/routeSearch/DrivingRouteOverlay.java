package com.bill.logisticsclouddemo.amap.routeSearch;
...
public class DrivingRouteOverlay extends RouteOverlay{
    ...
    public void refreshData(DrivePath path,LatLonPoint start, LatLonPoint end, List<LatLonPoint> throughPointList) {
        this.drivePath = path;
        startPoint = AMapUtil.convertToLatLng(start);
        endPoint = AMapUtil.convertToLatLng(end);
        this.throughPointList = throughPointList;
        ...
        try {
            ...
            for (int i = 0; i < drivePaths.size(); i++) {
                ...
                for (LatLonPoint latlonpoint : latlonPoints) {
                	mPolylineOptions.add(AMapUtil.convertToLatLng(latlonpoint));
                	mLatLngsOfPath.add(AMapUtil.convertToLatLng(latlonpoint));
				}
            }
...
//        texIndexList.add(++textIndex);
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            BitmapDescriptor bitmapDescriptor = getTrafficBitmapDescriptor(segmentTrafficStatus.getStatus());
        ...
        LogUtil.e("bitmapDescriptors.size()" + bitmapDescriptors.size() + ",texIndexList.size()"+texIndexList.size());
        mPolylineOptionscolor.setCustomTextureIndex(texIndexList);
        mPolylineOptionscolor.setCustomTextureList(bitmapDescriptors).useGradient(true);
    }
